...
# -*- coding: utf-8 -*-
"""
Created on Fri May 13 00:18:48 2022
...
class FGSBIR_Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'Train':
            ...
            img_sample = '_'.join(self.Train_Sketch[item].split('/')[-1].split('_')[:-1])
            image_path = os.path.join(self.root_dir,'photo',img_sample+'.png')
            ...
            sample = {'sketch_img': sketch_img, 'sketch_path': sketch_path,
                      'images': images, 'img_path': img_sample
                      }
            ...
            img_sample = '_'.join(self.Test_Sketch[item].split('/')[-1].split('_')[:-1])
            img_path = os.path.join(self.root_dir, 'photo', img_sample + '.png')
            images = self.test_transform(Image.open(img_path).convert('RGB'))
