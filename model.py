...
class FGSBIR_Model(nn.Module):
    def __init__(self, hp):
        ...
        self.loss = Rank_loss(anneal=0.01)
    ...
    def train_model(self, batch):
        ...
        query_feature = self.sample_embedding_network(batch['sketch_img'].to(device))
        preds = self.sample_embedding_network(batch['images'].to(device))

        loss = self.loss(query_feature,preds)
        loss.backward()
        self.optimizer.step()
